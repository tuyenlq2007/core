...
public abstract class PageVerificationApi {
	...
	public PageVerificationApi(String object_repository) {
	...
	/*
	 * public PageVerificationApi() { this.driver =
	 * DriverPool.INSTANCE.getDriver(SetUp.getSetup().getDriver());
	 * this.object_repository = ""; this.m_pro = new ContextualProperties();
	 * getProperties(SetUp.getSetup().getPropertyFile()); }
	 */
	...
	/*
	 * protected WebElement validateElement() throws Exception { WebElement
	 * element = null; String[] locators =
	 * m_pro.getProperty(object_repository).split(";"); int i = 0; while (i <
	 * locators.length - 1) { try { element = findElement(locators[i]); i =
	 * locators.length; } catch (Exception e) { // or your specific exception
	 * i++; } } element = findElement(locators[i]); return element; }
	 */
